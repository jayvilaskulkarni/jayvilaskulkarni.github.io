...
Then there is **unstructured** data. Think of product reviews on Amazon's website. The website can process the unformatted text from several reviews and generate a summary of sentiments and most relevant phrases. Political analysts and marketing folks use similar unstructured text data from social media platforms to analyse user sentiments on the topic of interest. Further, there is **semi-structured** data. This is data spread over several html pages of a websites. There is a pattern in which each html page of this website stores information, but it can't be downloaded directly (no API access either). You will then use webscraping techniques to create a structured spreadsheet out of this website's content. The webscraping process is the topic of this blog.
